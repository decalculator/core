...
async def main():
    ...
    for plg in await settings.get("plugins/content/plugins"):
        if await settings.get(f"plugins/content/plugins/{plg}/enabled") == True:
            if await settings.get(f"plugins/content/plugins/{plg}/entry_plugin") == True:
                entry_plugins.append(plg)

    size = len(entry_plugins)
    if size == 0:
        print("no entry ?")
    elif size == 1:
        plg = entry_plugins[0]
