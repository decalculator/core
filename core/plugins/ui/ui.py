...
class Ui:
    ...
    async def tasks_function(self):
        ...
        with dpg.window(label = "tasks", pos = [100, 0]):
            for obj, value in classic_running.items():
                if not obj in ["running", "to_run"]:
                    dpg.add_text(obj)
                    for unique_id in value:
                        dpg.add_checkbox(label = unique_id, callback = self.tasks_checkbox_callback, user_data = [obj, unique_id], default_value = True)
