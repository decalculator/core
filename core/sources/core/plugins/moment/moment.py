...
# Fichier : moment.py
# Rôle    : Ce fichier implémente la classe Moment, qui est utile
#           pour la gestion de moments.
...
# Import des modules requis par la classe Moment.
import asyncio
...
class Moment:
    ...
    def __init__(
        ...
        Rôle :
            Il s'agit de la méthode d'initialisation de la classe Moment,
    ...
    # Déclaration de la signature de la méthode next.
    def next(
        self,
        path: Path
    ):
        ...
        Entrée :
            path (Path) :
                Description : Le chemin de la valeur à incrémenter.

        Sortie :
            None

        Rôle :
            Incrémente de 1 la valeur située au chemin donné dans la
            structure JSON.
        """

        # Incrémente la valeur au chemin donné.
        self.data.write(path, self.data.get(path) + 1)

    # Déclaration de la signature de la méthode previous.
    def previous(
        self,
        path: Path
    ):
        ...
        Entrée :
            path (Path) :
                Description : Le chemin de la valeur à décrémenter.

        Sortie :
            None

        Rôle :
            Décrémente de 1 la valeur située au chemin donné dans la
            structure JSON.
        """

        # Décrémente la valeur au chemin donné.
        self.data.write(path, self.data.get(path) - 1)