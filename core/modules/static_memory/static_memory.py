...
class Static_memory:
    ...
    async def _create(self, name):
        ...
        await self.static_memory.write(Path(f"{name.json_path}/files"), {})
    ...
    async def get_structure(self, in_path, out_path, mode = 0):
        ...
        files = await self.path_utils.subfiles(path)

        for file in files:
            path = Path(f"{out_path.json_path}/{file.json_path}", mode = 1)
            if not await self.static_memory.exists(path):
                await self.static_memory.write(path, {}, 1)

            if mode == 1:
                await self.static_memory.write(Path(f"{path.json_path}/content"), await self.filesystem.read_file(file, 2))

    async def create(self, name, base_path = Path("core/data/static_memory", mode = 1)):
        main_directory_path = Path(f"{base_path.os_path}/{name.os_path}", mode = 1)
    ...
    async def write(self, path, value):
        resolved_path = Path(f"core/data/static_memory/{path}", mode = 1)
        splitted = path.split("/")
        ...
        for i in range(len(splitted) - 1):
            ...
            temp_path_obj = Path(temp_path, mode = 1)
            if not await self.filesystem.path_exists(temp_path_obj):
                await self.filesystem.mkdir(temp_path_obj)
